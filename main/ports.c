...
esp_err_t config_ports(void) {
    ...
    ESP_LOGI("PORTS", "Configurando START_INT (GPIO %d)...", START_INT);
    gpio_config_t start_config = {
        .pin_bit_mask = (1ULL << START_INT),
    ...
    ESP_LOGI("PORTS", "Configurando STOP_INT (GPIO %d)...", STOP_INT);
    gpio_config_t stop_config = {
        .pin_bit_mask = (1ULL << STOP_INT),
        .mode = GPIO_MODE_INPUT,
        .pull_up_en = GPIO_PULLUP_ENABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_NEGEDGE
    ...
    int start_level = gpio_get_level(START_INT);
    int stop_level = gpio_get_level(STOP_INT);
    ESP_LOGI("PORTS", "Níveis iniciais - START_INT: %d, STOP_INT: %d", start_level, stop_level);
    
    ESP_LOGI("PORTS", "Pinos de interrupção configurados: START_INT=%d, STOP_INT=%d", START_INT, STOP_INT);
