...
uint8_t ecg_logo[8][64] = {
    {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}, 
    {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 
    0x7f, 0x7f, 0x3f, 0x3f, 0x3f, 0x3f, 0x3f, 0x3f, 0x3f, 0x3f, 0x3f, 0x7f, 0x7f, 0xff, 0xff, 0xff, 
    0xff, 0xff, 0xff, 0x7f, 0x7f, 0x3f, 0x3f, 0x3f, 0x3f, 0x3f, 0x3f, 0x3f, 0x3f, 0x3f, 0x7f, 0x7f, 
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}, 
    {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x3f, 0x0f, 0x03, 0x01, 0x00, 
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xc0, 0xe0, 0x00, 0x00, 0x00, 0x01, 0x03, 
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xf0, 0xfe, 0x1f, 0x0f, 0x7f, 0xf8, 0x80, 0x00, 0x00, 
    0x00, 0x00, 0x00, 0x00, 0x80, 0xe0, 0xe0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
    ...
    0xf8, 0xd8, 0x98, 0x18, 0x18, 0x18, 0x1f, 0x07, 0x00, 0x00, 0x00, 0x00, 0x07, 0x3f, 0xfc, 0xc0, 
    0x00, 0xe0, 0xf8, 0x3e, 0x0f, 0x01, 0x0f, 0x1f, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0xd8, 0xf8, 
    0xf8, 0xf8, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}, 
    {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 
    0xff, 0xff, 0xff, 0xff, 0xfe, 0xfc, 0xf8, 0xf0, 0xe0, 0xc0, 0x80, 0x80, 0x00, 0x00, 0x01, 0x0f, 
    0x0f, 0x03, 0x00, 0x00, 0x00, 0x80, 0xc0, 0xe0, 0xf0, 0xf8, 0xf8, 0xfc, 0xfe, 0xff, 0xff, 0xff, 
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}, 
    {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 
...
{
    ESP_LOGI(SSD1306_TAG, "Escaneando dispositivos I2C...");
    
    for (uint8_t addr = 1; addr < 127; addr++) {
        esp_err_t ret = i2c_master_probe(i2c_master_bus, addr, 1000 / portTICK_PERIOD_MS);
        if (ret == ESP_OK) {
            ESP_LOGI(SSD1306_TAG, "Dispositivo encontrado no endereço 0x%02X", addr);
        }
    }
    ESP_LOGI(SSD1306_TAG, "Scan I2C completo");
}
...
void init_ssd1306(void)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(SSD1306_TAG, "Falha ao criar barramento I2C: %s", esp_err_to_name(ret));
        return;
    }
    
    ESP_LOGI(SSD1306_TAG, "Barramento I2C criado com sucesso");
    
    // Delay para estabilizar o barramento
    vTaskDelay(100 / portTICK_PERIOD_MS);
    
    // Agora sim: escanear dispositivos I2C (com barramento inicializado)
    i2c_scanner();
    
    // Tentar inicializar o SSD1306
    ESP_LOGI(SSD1306_TAG, "Tentando inicializar SSD1306...");
    ret = i2c_ssd1306_init(i2c_master_bus, i2c_ssd1306_config, &i2c_ssd1306);
    if (ret != ESP_OK) {
        ESP_LOGE(SSD1306_TAG, "Falha na inicialização SSD1306: %s", esp_err_to_name(ret));
        return;
    }
    
    ESP_LOGI(SSD1306_TAG, "SSD1306 inicializado com sucesso!");
    
    // Exibir logo e limpar
    i2c_ssd1306_buffer_image(&i2c_ssd1306, 32, 0, (const uint8_t *)ecg_logo, 64, 64, false);
