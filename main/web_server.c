...
static const char INDEX_HTML[] =
    ...
    "@media (min-width:640px){.controls{grid-template-columns:2fr 1fr 1fr 1fr}.controls .row{grid-template-columns:1fr 1fr 1f}}"
    "</style>"
    "</head><body>"
    "<div class='wrap'>"
    "  <div class='card'>"
...
static esp_err_t api_files_handler(httpd_req_t *req)
{
    ...
    while ((ent = readdir(dir)) != NULL)
    {
        ...
        // precisa começar com ECG_ e terminar com .bin
        if (!istartswith(name, "ECG_"))
            continue;
        if (strcasecmp(name + nlen - 4, ".bin") != 0)
            continue;
...
while ((n = fread(buf, 1, chunk_sz, f)) > 0) {
